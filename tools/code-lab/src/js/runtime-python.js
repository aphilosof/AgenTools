...
(function () {
  ...
  var WORKER_SRC = [
    ...
    "].join('\\n');",
    "var _outS, _errS;",
    "// Line-buffer raw stdout/stderr so each posted chunk is a COMPLETE line WITH its",
    "// newline (partials — prompts, end='' — flush without one). This makes the engine's",
    "// `stdout += s` reconstruct EXACT stdout; Pyodide's `batched` strips the newline.",
    "function _mkStream(kind) {",
    "  var dec = new TextDecoder(); var s = { pending: '' };",
    "  s.write = function (buf) {",
    "    s.pending += dec.decode(buf, { stream: true });",
    "    var i;",
    "    while ((i = s.pending.indexOf('\\n')) >= 0) {",
    "      postMessage({ type: kind, text: s.pending.slice(0, i + 1) });",
    "      s.pending = s.pending.slice(i + 1);",
    "    }",
    "    return buf.length;",
    "  };",
    "  s.flush = function () { if (s.pending) { postMessage({ type: kind, text: s.pending }); s.pending = ''; } };",
    "  s.reset = function () { s.pending = ''; };",
    "  return s;",
    "}",
    "async function init() {",
    "  pyodide = await loadPyodide({ indexURL: 'https://cdn.jsdelivr.net/pyodide/v0.29.4/full/' });",
    "  _outS = _mkStream('stdout'); _errS = _mkStream('stderr');",
    "  pyodide.setStdout(_outS); pyodide.setStderr(_errS);",
    ...
    "  if (e.data.type === 'run') {",
    "    if (_outS) _outS.reset(); if (_errS) _errS.reset();",
    "    try {",
    ...
    "      if (_outS) _outS.flush(); if (_errS) _errS.flush();",
    ...
    "    } catch (err) { if (_outS) _outS.flush(); if (_errS) _errS.flush(); postMessage({ type: 'error', text: String(err && err.message ? err.message : err) }); }",
    "  }",
    "};",
  ].join("\n");
