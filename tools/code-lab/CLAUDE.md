...
1. **Phase by phase.** Build strictly in the phase order of PLAN.md Section 6. Never start the next phase, or the next chapter within a content phase, without explicit user approval.
...
- **Scope sessions tightly.** One phase milestone or one chapter of content per session, ending at a clean commit. Do not start work that cannot reach a commit boundary.
...
- `lessons/`: one data file per chapter, conforming to `lessons/schema.md`.
...
- The stepper (`sys.settrace` line highlight plus live variable table) is core, not optional, and works on every Python exercise from Chapter 1.
...
- Errors: the real traceback always displays untouched, annotation beneath it, fading per PLAN.md Section 3 (full through Chapter 4, on demand after Chapter 5, off from Real Tools I, never in the Arena). Every translated error class registers in the Codex errors tab.
- Two feedback channels: tolerant diagnostic correctness checks that never produce a bare fail, and a separate style channel (naming, magic numbers, repetition, function length) active from Chapter 5, blocking only on Chapter 6+ checkpoints.
...
- Conventional commits, one commit per coherent unit (a feature, a chapter of content).
