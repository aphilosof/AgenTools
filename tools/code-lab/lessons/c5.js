...
window.CODELAB.lessons.push({
  ...
  content: [
    ...
    {
      type: "text",
      md: "**Charting real data, decomposed.** A week of real temperatures, charted with Lesson 4.7's `bar()`, and a second function reporting the hottest day — the same one-function-one-job split as everything else in this chapter.",
    },
    {
      type: "example",
      note: "chart_week draws the bars. Nothing computes the hottest day yet -- that's the next exercise.",
      code: "week_temps = {\"Mon\": 68, \"Tue\": 71, \"Wed\": 74, \"Thu\": 70, \"Fri\": 65}\n\ndef chart_week(data):\n    bar(list(data.keys()), list(data.values()))\n\nchart_week(week_temps)\n",
    },
    {
      type: "exercise",
      rung: 5,
      prompt: "Complete `hottest_day(data)` — it should return the key (day name) with the highest temperature. `chart_week` and `describe_week` are already written and call it.",
      starter: "week_temps = {\"Mon\": 68, \"Tue\": 71, \"Wed\": 74, \"Thu\": 70, \"Fri\": 65}\n\ndef chart_week(data):\n    bar(list(data.keys()), list(data.values()))\n\ndef hottest_day(data):\n    pass  # return the key with the highest value\n\ndef describe_week(data):\n    chart_week(data)\n    print(\"Hottest day:\", hottest_day(data))\n\ndescribe_week(week_temps)\n",
      check: { type: "output", expected: "Hottest day: Wed" },
      hints: [
        "This is the same max-with-key pattern from Lesson 5.1's most_played.",
        "max(data, key=data.get) returns the KEY with the highest value.",
        "return max(data, key=data.get)",
      ],
    ...
    },
    {
      type: "exercise",
      rung: 6,
      prompt: "Write `temp_melody(data)` from scratch. For each day's temperature in `week_temps` (in dictionary order), play that temperature as a note, sleeping 0.3 seconds between notes. Then print the week's average temperature, rounded to 1 decimal place.",
      ...
      hints: [
        ...
        "play(temp) then sleep(0.3), same shape as play_scale earlier in this lesson.",
        "return or print round(sum(data.values()) / len(data), 1) after the loop.",
      ],
