...
window.CODELAB.lessons.push({
  ...
  content: [
    ...
    },
    {
      type: "exercise",
      rung: 5,
      prompt: "Complete `play_bass(root, drop)` — it should play one note, `root + drop`, then sleep 0.4 seconds. `play_arpeggio` and `perform` are already written and call it.",
      starter: "root = 60\nchord = [0, 4, 7, 12]\nbass_drop = -12\n\ndef play_arpeggio(intervals, root):\n    for i in intervals:\n        play(root + i)\n        sleep(0.4)\n\ndef play_bass(root, drop):\n    pass  # play root + drop, then sleep 0.4\n\ndef perform(intervals, root, drop):\n    play_bass(root, drop)\n    play_arpeggio(intervals, root)\n\nperform(chord, root, bass_drop)\n",
      check: {
        type: "calls",
        calls: [
          { fn: "play", note: 48 },
      ...
      hints: [
        "The bass note is root + drop, played once — one play() call, one sleep() call.",
        "root is 60 and drop is -12, so the bass note is 48.",
        "play(root + drop) then sleep(0.4) — that's the whole function body.",
      ],
      solution: "root = 60\nchord = [0, 4, 7, 12]\nbass_drop = -12\n\ndef play_arpeggio(intervals, root):\n    for i in intervals:\n        play(root + i)\n        sleep(0.4)\n\ndef play_bass(root, drop):\n    play(root + drop)\n    sleep(0.4)\n\ndef perform(intervals, root, drop):\n    play_bass(root, drop)\n    play_arpeggio(intervals, root)\n\nperform(chord, root, bass_drop)\n",
    },
    {
    ...
    },
    {
      type: "exercise",
      rung: 5,
      prompt: "Complete `total_moons(data)` — it should return the sum of every planet's moon count. `chart_moons` and `summarize_moons` are already written and call it.",
      starter: "moons = {\"Saturn\": 146, \"Jupiter\": 95, \"Uranus\": 27, \"Neptune\": 16, \"Mars\": 2, \"Earth\": 1}\n\ndef chart_moons(data):\n    bar(list(data.keys()), list(data.values()))\n\ndef total_moons(data):\n    pass  # return the sum of every value in data\n\ndef summarize_moons(data):\n    chart_moons(data)\n    print(\"Total moons:\", total_moons(data))\n\nsummarize_moons(moons)\n",
      check: { type: "output", expected: "Total moons: 287" },
      hints: [
        "sum() works directly on the dictionary's values.",
        "data.values() gives you every moon count without the planet names.",
